...
'''
q_learning is a temporal differential off(on)-policy
q_{t+1}(s_t, a_t) = q_t(s_t, a_t) - alpha_t(s_t, a_t) * (q_t(s_t, a_t) - [r_{t+1} + gamma * q_t(s_{t+1}, a_{t+1}})])
'''
class QLearning(ValueIter):
    def learning(self, start_state=0, start_action_idx=0, exps_n=1, max_length=10000):
        ...
        behavior_policy = Policy(len(self.maze), self.maze._actions_set)
        ...
        for i in range(exps_n):
            ...
            while j < max_length and self.maze.maze[curr_state // self.maze.cols][curr_state % self.maze.cols] != "t":
                j += 1
                visit = self.maze.gen_exp(curr_state, curr_action, behavior_policy)
                ...
                td_target = reward + self.gamma * np.max(self.action_values[next_state])
...
if __name__ == "__main__":
    ...
    ql = QLearning(maze, eps_greedy=1)
    ql.print()

    print()
    ql.learning(exps_n=1000)
